...
const FilterTabs = ({filters, setFilters}: PropTypes) => {
  ...
  const filterChange = (currFilters: Filters, newFilter: FilterTypes) => {
    if(!currFilters){
      ...
      const productExists = Boolean(currFilters.find((item) => item === newFilter));
      if(productExists){
  ...
  const isFilterSelected = useCallback((filter: FilterTypes) => {
    if(!filters){
      return false
    }
  ...
  return (
    <div className='flex divide-x divide-slate-700 border-b border-slate-700'>
      <div className='flex flex-1 divide-x divide-slate-700 justify-center'>
        <div onClick={handleFIlterChange('comercial')} className={`w-full px-10 py-5 text-[16px] cursor-pointer hover:bg-red-500 ${isFilterSelected('comercial') ? 'bg-red-500': ''}`}>
          Comercial
        </div>
        <div onClick={handleFIlterChange('residential')} className={`w-full px-10 py-5 text-[16px] cursor-pointer hover:bg-red-500 ${isFilterSelected('residential') ? 'bg-red-500': ''}`}>
          Residential
        </div>
      </div>
      <div className='flex w-[65vw] justify-between divide-x divide-slate-700'>
        <div className='flex flex-1'>
          <div className='flex px-8 text-[16px] underline items-center'>Clear all</div>
        </div>
